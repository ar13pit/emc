...
void WorldModel::set_updateMappingVariables () {
    ...
    temp.angle = currentMappingData_.pico_position.angle;
...
Position WorldModel::get_globalPosition () {
...
std::vector<Point_map> WorldModel::get_allDetectedCorners(){
    return allDetectedCorners_;
}

std::vector<Exit_map> WorldModel::get_allDetectedExits(){
    return allDetectedExits_;
}
...
void WorldModel::set_globalPosition (Position updatedGlobalPosition) {
...
void WorldModel::set_closestRoom(){
    ...
    Position curPos = get_globalPosition();
