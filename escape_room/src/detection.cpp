...
#include "config.h"
    ...
    Exit Detection::findExit(){
                    ...
                    if (nLarger > nPoint_detsThresh){ // Exit detected
                        ...
                        for (int k = j; k < 1000 - 2*15 - nPoint_detsSearch; ++k){
                            ...
                            if (nEqual > 5){
                               iExit2 = k - 5;
                               Exit exit;
                               exit.exitPoint_det1 = LatestLaserScan[iExit1];
                               exit.exitPoint_det2 = LatestLaserScan[iExit2];
                               exit.detected = true;
                               return exit;
                            }
                        }
        ...
        Exit exit;
        exit.exitPoint_det1 = LatestLaserScan[0];
        exit.exitPoint_det2 = LatestLaserScan[0];
        exit.detected = false;
        return exit;
    ...
    Point_det Detection::findFurthestPoint_det(){
        int imax = 0;
        double max = 0;
